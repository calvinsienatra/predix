...
# =======================================================
def convert_profile(profile, indexer, class_indexer):
	keys = profile.keys()
	mylist = []
	label = ''
...
def main(arg):
	...
	try:
		#path = os.getcwd()
		arr_indexer = open(path+'/victoriaMachineLearning/indexer.csv', 'r').readline().split(',')
	...
	arr_class = open(path+'/victoriaMachineLearning/class_indexer.csv', 'r').readline().split(',')
	class_indexer = Indexer(arr_class)
	# print("here2")

	train_X, train_y = read_file_to_data(path+'/victoriaMachineLearning/training.csv')
	test_X, test_y = read_file_to_data(path+'/victoriaMachineLearning/test.csv')

	new_model = joblib.load(path+'/victoriaMachineLearning/model.pkl')
	...
	arg = json.loads(arg)
	# print(arg)
	converted = convert_profile(arg, indexer, class_indexer)
	# print(converted)
	p_instance = model.predict_instance(converted)
	print("Most likely company you work in:", class_indexer.get_object(p_instance))
	...
	# print(test_X[-1])
	probs = model.predict_prob_instance(converted)

	for i, prob in enumerate(probs):
		print("Probability of working in " + str(class_indexer.get_object(i)) + " is " + str(prob))

	print("You want to work in " + str(class_indexer.get_object(8)))
	advice = model.get_advice(converted, 8)
	arr = sorted([indexer.get_object(i) for i in advice])
	print("You need to improve the following skills:")
	for x in arr:
		print(x)
